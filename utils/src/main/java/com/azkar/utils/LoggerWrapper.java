...
public final class LoggerWrapper {
    ...
    private static final DateTimeFormatter FILE_NAME_TIME_FORMAT = DateTimeFormatter.ofPattern("dd_MM_yyyy_HH_mm");
    ...
    private static void installConfiguredHandlers() {
        ...
        if (fileOn) {
            try {
                Path dir = Path.of(ENV_PROPS.getProperty(KEY_LOG_DIR, DEFAULT_LOG_DIR))
                        .toAbsolutePath();
                Files.createDirectories(dir);

                String file = "root-" + LocalDateTime.now().format(FILE_NAME_TIME_FORMAT) + ".log";

                FileHandler fh = new FileHandler(dir.resolve(file).toString(), true);
                fh.setLevel(Level.ALL);
                fh.setFormatter(new LogFormatter());

                ROOT_LOGGER.addHandler(fh);
            } catch (IOException e) {
                System.err.println("Failed to create root log file: " + e.getMessage());
            }
        }
    }
    ...
    private static void applyRootLevel() {
        ...
        String lvl = ENV_PROPS
                .getProperty(KEY_LOG_LEVEL, Level.INFO.getName())
                .trim()
    ...
    private static void attachPerLoggerFileHandlerIfEnabled(Class<?> ownerClass, Logger logger) {
        ...
        Path dir = Path.of(base, ownerClass.getPackageName(), ownerClass.getSimpleName());

        try {
            Files.createDirectories(dir);

            FileHandler fh = new FileHandler(dir.resolve(time + ".log").toString(), true);
            fh.setLevel(Level.ALL);
            fh.setFormatter(new LogFormatter());

            logger.addHandler(fh);
        } catch (IOException e) {
            ROOT_LOGGER.log(Level.SEVERE, "Failed to create per-logger file: " + dir + " (" + e.getMessage() + ")", e);
        }
    }
    ...
    private static Properties loadDotEnvThenEnvThenProps() {
        ...
        if (Files.isRegularFile(dotEnv)) {
            try (InputStream in = Files.newInputStream(dotEnv)) {
                ...
                System.err.println("LoggerWrapper: Could not load .env: " + e.getMessage());
            }
        }

        // 2) env vars override .env
        Set<String> logSettingsKeys = Set.of(KEY_ENABLE_CONSOLE, KEY_ENABLE_FILE, KEY_LOG_LEVEL, KEY_LOG_DIR);

        System.getenv().entrySet().stream()
                .filter(e -> e.getValue() != null && logSettingsKeys.contains(e.getKey()))
        ...
        System.getProperties().entrySet().stream()
                ...
                .forEach(e -> p.setProperty(e.getKey().toString(), e.getValue().toString()));
