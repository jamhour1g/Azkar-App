...
class InstantEpochConverterTest {
    ...
    @DisplayName(
        "Should convert epoch seconds to Instant and back consistently"
    )
    void roundTrip_fromEpochSeconds_preservesValue(long epochSeconds) {
        ...
        assertEquals(epochSeconds, backToDb, () ->
            "Round-trip mismatch for epoch seconds=%d. Converted Instant=%s, Back to DB=%d".formatted(
                epochSeconds,
                instant,
                backToDb
            )
    ...
    void roundTrip_fromInstant_withNanos_truncatesToSeconds() {
        ...
        assertEquals(original.getEpochSecond(), dbValue, () ->
            "Expected DB value to equal epochSecond=%d but was %d for Instant=%s".formatted(
        ...
        assertEquals(expected, roundTripped, () ->
            "Expected round-tripped Instant to be truncated-to-seconds=%s but was %s (original=%s)".formatted(
                expected,
                roundTripped,
                original
            )
        );
        assertEquals(0, roundTripped.getNano(), () ->
            "Expected nanos to be 0 after round-trip but were %d".formatted(
                roundTripped.getNano()
            )
        );
    }
    ...
    @Test
    @DisplayName(
        "Should convert Instant.now() to DB and back with second-level precision"
    )
    ...
    void convertToDatabaseColumn_nullInstant_returnsNull() {
        ...
        assertNull(
            result,
            "Expected result to be null when converting null Instant"
        );
    }
    ...
    void convertToEntityAttribute_nullLong_returnsNull() {
        ...
        assertNull(
            result,
            "Expected result to be null when converting null DB value"
        );
    }

    @Test
    @DisplayName(
        "Should throw DateTimeException for out-of-range epoch seconds"
    )
    void convertToEntityAttribute_outOfRange_throwsDateTimeException() {
        assertThrows(
            ...
            () ->
                "Expected DateTimeException for out-of-range epoch seconds=%d".formatted(
                    Long.MAX_VALUE
                )
        );
    }
