...
public final class RemembranceRepositoryAdapter implements RemembranceRepository {

    private final RemembranceDataRepository dataRepo;
    private final EntityManager em;

    public RemembranceRepositoryAdapter(RemembranceDataRepository dataRepo, EntityManager em) {
    ...
    public Remembrance save(Remembrance remembrance) {
        try {
            RemembranceEntity entity = RemembranceMapper.fromRemembrance(remembrance);
            ...
            throw new PersistenceFailureException("Failed to save remembrance", e);
        }
    }
    ...
    public void delete(Remembrance r) {
        ...
        try {
            RemembranceEntity ref =
                    em.getReference(RemembranceEntity.class, r.getId().get());
            dataRepo.delete(ref);
        } catch (PersistenceException e) {
            throw new PersistenceFailureException(
                    "Failed to delete remembrance id=" + r.getId().get(), e);
        }
    }
    ...
    public void deleteById(long id) {
        try {
            em.createQuery("DELETE FROM RemembranceEntity r WHERE r.id = :id")
                    .setParameter("id", id)
                    .executeUpdate();
        } catch (PersistenceException e) {
            throw new PersistenceFailureException("Failed to delete remembrance id=" + id, e);
        }
    }
    ...
    public Optional<Remembrance> findById(long id) {
        try {
            ...
            throw new PersistenceFailureException("Failed to find remembrance id=" + id, e);
        }
    }
    ...
    public List<Remembrance> findAll() {
        try {
            ...
            throw new PersistenceFailureException("Failed to find all remembrances", e);
        }
    }
    ...
    public List<Remembrance> findByTagNameIgnoreCase(String tagName) {
        try {
            return dataRepo.findByTagNameIgnoreCase(tagName).stream()
                    .map(RemembranceMapper::toRemembrance)
                    .toList();
        } catch (PersistenceException e) {
            throw new PersistenceFailureException("Failed to find remembrances by tag '" + tagName + "'", e);
        }
    }
    ...
    public List<Remembrance> findFavorites() {
        try {
            return dataRepo.findFavorites().stream()
                    .map(RemembranceMapper::toRemembrance)
                    .toList();
        } catch (PersistenceException e) {
            throw new PersistenceFailureException("Failed to find favorite remembrances", e);
        }
    }
    ...
    public List<Remembrance> search(Locale locale, String expressionToSearchFor) {
        ...
        try {
            String pattern = "%" + JpqlUtils.escapeLikePattern(expressionToSearchFor) + "%";
            return dataRepo.searchByTranslationText(locale, pattern).stream()
                    .map(RemembranceMapper::toRemembrance)
                    .toList();
        } catch (PersistenceException e) {
            throw new PersistenceFailureException("Failed to search remembrances", e);
        }
    }
    ...
    public void markFavorite(long remembranceId) {
        try {
            RemembranceEntity entity = em.find(RemembranceEntity.class, remembranceId);
            if (entity == null) {
                throw new RemembranceNotFoundException(remembranceId);
            }
            entity.markFavorite();
        } catch (PersistenceException e) {
            throw new PersistenceFailureException("Failed to mark remembrance id=" + remembranceId + " as favorite", e);
        }
    }
    ...
    public void unmarkFavorite(long remembranceId) {
        try {
            ...
            throw new PersistenceFailureException(
                    "Failed to unmark remembrance id=" + remembranceId + " as favorite", e);
        }
    }
}
